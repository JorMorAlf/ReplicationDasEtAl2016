...
* PANEL B: Regressions with SP fixed effects, district/provider fixed effects	
*** Alternative model: logit instead of lpm

foreach yvar in correct_treat helpful_treat wrong_treat correct_only antibiotic totalmeds{

	* Dual
	logit  `yvar' private case3 ssp2-ssp15 i.distid  if dual==1 & round==2 & case!=2, robust cl(finprovid)
	margins , dydx(priv)
	est store s2log_`yvar'
	
	}
	
	xml_tab s2log_correct_treat s2log_helpful_treat s2log_wrong_treat s2log_correct_only s2log_antibiotic s2log_totalmeds s2log_correct_treat s2log_helpful_treat s2log_wrong_treat s2log_correct_only s2log_antibiotic s2log_totalmeds, ///
	save("Output/Table5_logit.xml") append sheet(Table5_Logit) font("garamond" 11) ///
	keep(private) below stats(r2 N) mv("") format(SCLR0 (SCCR0 NCCR3 NCCR3))

	* PANEL C: Regressions with controls, SP fixed effects, district/provider fixed effects	
*** Alternative model: adding inputed age variable to reduce the missing data 
	sum age if round==2 & dual==1 
gen age2=age
replace age2=45.10927 if age==. & round==2 & dual==1 

foreach yvar in correct_treat helpful_treat wrong_treat correct_only antibiotic totalmeds {

	* Audit 1
	reg `yvar' private case3 mbbs qual age2 gender numafter ssp2-ssp15 market* if round==1 & case!=2 [weight=wt], robust cluster(villid) 
	est store s1_`yvar'
	
	* Dual
	areg `yvar' private case3 age2 gender numafter ssp2-ssp15 if dual==1 & round==2 & case!=2, absorb(distid) robust cl(finprovid)
	est store s2_`yvar'
	
	}
	
		xml_tab s2log_correct_treat s2log_helpful_treat s2log_wrong_treat s2log_correct_only s2log_antibiotic s2log_totalmeds s2log_correct_treat s2log_helpful_treat s2log_wrong_treat s2log_correct_only s2log_antibiotic s2log_totalmeds, ///
	save("Output/Table5_ageinput.xml") append sheet(Table5_age) font("garamond" 11) ///
	keep(private) below stats(r2 N) mv("") format(SCLR0 (SCCR0 NCCR3 NCCR3))
